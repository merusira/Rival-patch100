...
class Monitor {
    ...
    constructor(mod, mods) {
        ...
        this._minPing = 11;                 // Track minimum ping value to avoid recalculation
    ...
    addPingResult(ping) {
        ...
        ping = Number(ping);
        this._ping_array.push(ping);
        
        // Update minimum ping value
        if (ping < this._minPing || this._ping_array.length === 1) {
            this._minPing = ping;
            this.ping = this._minPing;
        }
        
        // Reduce the array if it's over the max size
        if (this._ping_array.length > this.CONFIG.MAX_PING_CACHE_SIZE) {
            const removedPing = this._ping_array.shift();
            
            // If we removed the minimum value, recalculate the minimum
            if (removedPing === this._minPing) {
                this._minPing = Math.min(...this._ping_array);
                this.ping = this._minPing;
            }
        }
    ...
    destructor() {
        ...
        try {
            ...
            this.mod.unhook('S_SPAWN_ME');
            this.mod.unhook('S_LOAD_TOPO');
            this.mod.unhook('S_RETURN_TO_LOBBY');
            this.mod.unhook('C_REQUEST_GAMESTAT_PING');
            this.mod.unhook('S_RESPONSE_GAMESTAT_PONG');
        } catch (e) {
