...
class Skills extends EventEmitter {
    ...
    canCast = (skillData, {
        ...
        if (!isKeepingMovingCharge) {
            for (const category of skillInfo.categories) {
                if (this.mods.effects.isCategoryEnabled(category)) continue;
        ...
        // Check cooldown
        if (this.mods.cooldown.isOnCooldown(skillId, originalSkillId)) return -12; // On cooldown
        
        // Check weapon requirement
        if (!skillInfo.noNeedWeapon && !this.mods.player.inven.weapon) return -13; // No weapon
        ...
        if (type !== 5 && skillInfo.typeId === 27) return -27; // Type mismatch
        ...
        if (this.mods.player.job === 4 && skillId === 359096) return -16;
        ...
        // Check various skill type conditions
    ...
    _calculateTime = (baseTime, elapsedTime, speedFactor, overrideSkillId = null, overrideStage = null, useWiggleRoom = true) => {
        ...
        // Get animation length for all stages
        const animLength = this.getAnimationlengthForAllStages(skillId, {
            real: 1
        }, stage);
        
        timings.push(animLength);
        
        // Find closest timing
        const adjustedElapsedTime = elapsedTime - wiggleRoom;
        const differences = timings.map(time => Math.abs(time - adjustedElapsedTime));
        const sortedDifferences = [...differences].sort((a, b) => a - b);
        const closestTime = timings[differences.indexOf(sortedDifferences[0])];
        const calculatedTime = Math.floor((elapsedTime - closestTime - wiggleRoom) / speedFactor) - 1;
        const isUsingBaseTime = closestTime === baseTime;
        
        // Handle edge cases with negative calculated time
        if (isUsingBaseTime ? calculatedTime <= wiggleRoom * -1 : calculatedTime <= -8) {
            for (const diff of sortedDifferences) {
                const time = timings[differences.indexOf(diff)];
                const altCalculatedTime = Math.floor((elapsedTime - time - wiggleRoom) / speedFactor) - 1;
                if (altCalculatedTime >= 0) return altCalculatedTime;
            }
            return 0;
        }
    ...
    getNewSkillData = (requestedSkillId, {
        ...
        if (!byGrant &&
            ...
            [90100, 90130].includes(requestedSkillId) &&
            this.mods.effects.getPassivity(32065)) {
            requestedSkillId = 90131;
        }
        
        // Apply skill transformations from effects
        const appliedEffects = this.mods.effects.getAppliedEffects(requestedSkillId);
        if (appliedEffects.transform) {
            requestedSkillId = appliedEffects.transform;
        }
        ...
        do {
            ...
            for (const {
            ...
            } of skillInfo?.abnormalityRedirect || []) {
                ...
                // If the redirect is too far, recursively call getNewSkillData
                if (Math.abs(requestedSkillId - redirectSkillId) > LARGE_SKILL_ID_DIFFERENCE) {
                    return this.getNewSkillData(redirectSkillId, {
                        byGrant,
                        press
                    });
                }
                ...
                requestedSkillId = redirectSkillId;
                skillInfo = redirectSkillInfo;
                break;
            }
        ...
        if ([classes.NINJA, classes.BERSERKER].includes(this.mods.player.job) &&
            hasAbnormality &&
            skillInfo.typeId === 28) {
            this.mods.log.debug("NEW-SKILL-DATA", "Removing abnormality chain (28): " + requestedSkillId);
        ...
        // Handle connect skill arrow
        if (byGrant && this.info.connectSkillArrow[originalSkillId]) {
        ...
        // Set up timing variables for cancel checks
        const {
            rearStartTime = -1,
            pendingStartTime = -1,
            front = -1
        ...
        // Check if skill can be rear-canceled
