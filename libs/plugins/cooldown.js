 ...
 *
 * plugins/cooldown.js serves as a skill cooldown management system.
...
module.exports = function Cooldown(mod, mods) {
    ...
    mod.hook(...mods.packet.get_all("S_ACTION_STAGE"), hooks.READ_DESTINATION_FAKE, event => {
        ...
        cooldownTimeoutId = mod.setTimeout(() => {
            ...
            // Enhanced logging for multi-stage skill cooldown application
            const isMultiStageSkill = skillInfo && mods.skills._getInfo(skillInfo.id)?.nextSkill;
            const groupId = `${Math.floor(skillInfo.skill / 10000)}-0`;
            const skillType = mods.skills.getType(skillInfo.id);
            
            // Check if this is a projectile, drain, lockon, or movingSkill-type skill
            // These skills should allow other skills to be used while they're active
            const isProjectileSkill = skillType === "projectile";
            const isDrainSkill = skillType === "drain";
            const isMovingSkill = skillType === "movingSkill";
            const hasLockon = mods.skills.getLockonData(skillInfo.id);
            const isSpecialSkill = isProjectileSkill || isDrainSkill || hasLockon || isMovingSkill;
            
            if (isMultiStageSkill) {
                mods.log.debug("COOLDOWN_DEBUG", `Applying cooldown to multi-stage skill: ${skillInfo.id}`);
                mods.log.debug("COOLDOWN_DEBUG", `Skills to apply cooldown to: ${JSON.stringify(skillsToApplyCooldownTo)}`);
                mods.log.debug("COOLDOWN_DEBUG", `Group ID: ${groupId}, Skill type: ${skillType}`);
                mods.log.debug("COOLDOWN_DEBUG", `Is projectile: ${isProjectileSkill}, Is drain: ${isDrainSkill}, Is movingSkill: ${isMovingSkill}, Has lockon: ${hasLockon}`);
                mods.log.debug("COOLDOWN_DEBUG", `Cooldown time: ${cooldownData.cooltime}ms`);
            }
            ...
            // For multi-stage skills, apply cooldown to the group ID only if it's not a special skill type
            // This allows players to use other skills while projectiles are in flight or drain skills are active
            if (isMultiStageSkill && groupId && !isSpecialSkill) {
                mods.log.debug("COOLDOWN_DEBUG", `Also applying cooldown to group ID: ${groupId}`);
                mod.send(...mods.packet.get_all("S_START_COOLTIME_SKILL"), {
                    skill: groupId,
                    cooldown: cooldownData.cooltime
                });
            } else if (isMultiStageSkill && isSpecialSkill) {
                mods.log.debug("COOLDOWN_DEBUG", `Not applying group cooldown for special skill ${skillInfo.id} (type: ${skillType}, is movingSkill: ${isMovingSkill}, has lockon: ${hasLockon})`);
            }
